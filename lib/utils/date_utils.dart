...
class MDateUtils {
  ...
  static String dateToFormattedDate(String? date, bool showYear, {bool? full, bool generic = false}) {
    if(date == null) return '';
    DateTime? dateTime = DateTime.tryParse(date)?.toLocal();
    String formattedDate;
    if(dateTime == null) return date ?? '';
    generic? formattedDate = DateFormat('dd MMM yyyy').format(dateTime) :
    showYear
  ...
  static String dateToQueryDate(String? date,) {
    if(date == null) return '';
    DateTime? dateTime = DateTime.tryParse(date)?.toLocal();
    String formattedDate;
    if(dateTime == null) return date ?? '';
  ...
  static DateTime queryDateToDate(String? date,) {
    return DateFormat('MM/dd/yyyy').parse(date ?? '01/11/2000');
  }
  static String dateToFormattedDateAlt(String? date,  {bool? full, bool generic = false, bool showYear = true}) {
    if(date == null) return '';
    try{
  ...
  static String timeToHourMinute(String? date) {
    ...
    if(liz.length == 3){
      DateTime dateTime = DateTime(now.year, now.month, now.day, liz[0], liz[1], liz[2]);
      return DateFormat('hh:mm aa', 'en-IN').format(dateTime).toUpperCase();
    }
    return date ?? '';
  }
